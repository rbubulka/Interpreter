...
(define parse-exp         
  (lambda (datum)
    (cond
	 ...
	 [(eqv? (1st datum) 'for) (for-exp (map parse-exp (1st (2nd datum))) (parse-exp (3rd (2nd datum))) (map parse-exp (cddddr (2nd datum))) (map parse-exp (cddr datum)))]
...
(define syntax-expand (lambda (exp)
  (cases expression exp
    ...
    [for-exp (inits test update bodies) (syntax-expand (begin-exp 
							(append inits 
								(list (named-let-exp 'for-loop '() '() 
										     (list (if-exp test
											     (begin-exp (append bodies
														update
														(list (app-exp (parse-exp 'for-loop)
															       (list))))))))))))]
    [else exp]
    )))
